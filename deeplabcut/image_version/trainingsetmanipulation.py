...
def label_frames(config):
    ...
    from deeplabcut.image_version import labeling_toolbox
...
def MakeLabeledPlots(folder,DataCombined,cfg,Labels,Colorscheme,cc,scale):
    ...
    for index, imagename in enumerate(DataCombined.index.values):
        image = io.imread(os.path.join(folder,imagename))
        plt.axis('off')
...
def merge_and_split_files(config, windows2linux=False, train_images=None, test_images=None, trainindex = 0):
    """
    ...
    One can split the images by the path/label of the images in training and test images
    If training or test images is set to None, then the complement set is used as either training or test set
    ...
    trainindex: int, optional
        Used if training and test images are set to None
        Used to specify which which training fraction should be used of config.yaml
    ...
    train_images: list
        Uses index in the labels of the labeled-data
        Specify which images should be used for the training set of images

    test_images: list
        Uses index in the labels of the labeled-data
        Specify which images should be used for the test set of images


    Examples
    --------
    Creates a shuffle with training image 01.jpg and test image 02.jpg if they exist as labeled images
    >>> trainIndexes, testIndexes=deeplabcut.mergeandsplit(config,train_images=["01.jpg"],test_images=["02-jpg"])
    returns the indices for the test and training images of the merged labels from the directories.
    ...
    print("Using the following images for training:", train_images)
    print("Using the following images for testing:", test_images)
    ...
    if train_images is None and test_images is None:
        ...
        if train_images is None:
            train_images = list(set(Data.index) - set(test_images))
        elif test_images is None:
            test_images = list(set(Data.index) - set(train_images))
        for index, name in enumerate(Data.index):
            # print(index,name.split(os.sep)[1])
            if name in train_images:  # this is the video name
                # print(name,test_video_name)
                trainIndexes.append(index)
            elif name in test_images:
...
def create_training_dataset(config,num_shuffles=1,Shuffles=None,windows2linux=False,trainIndices=None,testIndices=None):
    """
    Creates a training dataset. Labels from all the extracted images are merged into a single .h5 file.\n
    ...
    if cfg['resnet']==50:
        ...
        resnet_path = str(os.path.join(os.path.dirname(__file__),'..' ,'pose_estimation_tensorflow', 'models', 'pretrained', 'resnet_v1_50.ckpt'))
    elif cfg['resnet']==101:
        net_type ='resnet_'+str(cfg['resnet'])
        #resnet_path = str(Path(deeplabcut.__file__).parents[0] / 'pose_estimation_tensorflow/models/pretrained/resnet_v1_101.ckpt')
        resnet_path = str(os.path.join(os.path.dirname(__file__),'..' , 'pose_estimation_tensorflow', 'models', 'pretrained', 'resnet_v1_101.ckpt'))
    else:
        ...
        for trainFraction in TrainingFraction:
            ...
            if len(trainIndexes)>0:
                ...
                defaultconfigfile = str(
                    os.path.join(os.path.dirname(os.path.dirname(__file__)), 'pose_cfg.yaml'))
