...
def get_list_of_images(images, imagetype):
    from random import sample
    # checks if input is a directory
    if [os.path.isdir(i) for i in images] == [True]:  # os.path.isdir(video)==True:
        """
        Analyzes all the videos in the directory.
        """

        print("Analyzing all the videos in the directory")
        imagefolder = images[0]
        os.chdir(imagefolder)
        imagelist = [fn for fn in os.listdir(os.curdir) if
                     (imagetype in fn) and ('labeled.mp4' not in fn)]  # exclude labeled-videos!
        Images = sample(imagelist,
                        len(imagelist))  # this is useful so multiple nets can be used to analzye simultanously
    else:
        if isinstance(images, str):
            if os.path.isfile(images):  # #or just one direct path!
                Images = [v for v in images if os.path.isfile(v) and ('labeled.mp4' not in v)]
            else:
                Images = []
        else:
            Images = [v for v in images if os.path.isfile(v) and ('labeled.mp4' not in v)]
